...
class ViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    dot.borderColor = UIColor.init(white: 1.0, alpha: 1.0).CGColor
    dot.borderWidth = 0.5
    dot.cornerRadius = 1.5
    replicator.addSublayer(dot)
    
    replicator.instanceCount = Int(view.frame.width / dotOffset) + 2 // + 2 -> add somes
  ...
  @IBAction func actionStartMonitoring(sender: AnyObject) {
    ...
    monitor.startMonitoringWithHandler { level in
        self.meterLabel.text = String(format: "%.2f", level)
  ...
  func startSpeaking() {
    ...
    initialRotation.repeatCount = Float.infinity
    initialRotation.autoreverses = true
